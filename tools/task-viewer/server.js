...
const SETTINGS_FILE = path.join(
  os.homedir(),
  ".shrimp-task-viewer-settings.json"
);
const GLOBAL_SETTINGS_FILE = path.join(
  os.homedir(),
  ".shrimp-task-viewer-global-settings.json"
);
    ...
    for (const functionName of functionDirs) {
      ...
      if (stat.isDirectory()) {
        ...
        try {
          ...
          console.log(
            `No index.md found in custom templates for ${functionName}`
          );
        }
      }
    }
...
async function startServer() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (url.pathname === "/api/check-env" && req.method === "GET") {
      ...
      console.log(
        `Checking env var ${envVarName}: ${isSet ? "SET" : "NOT SET"}`
      );
      ...
      url.pathname.startsWith("/api/remove-project/") &&
      req.method === "DELETE"
    ) {
      ...
      url.pathname.startsWith("/api/rename-project/") &&
      req.method === "PUT"
    ) {
      const projectId = url.pathname.split("/").pop();
      let body = "";
      ...
      url.pathname.startsWith("/api/update-project/") &&
      req.method === "PUT"
    ) {
      const projectId = url.pathname.split("/").pop();
      let body = "";
      ...
      try {
        ...
        try {
          ...
          const emptyResponse = {
            ...
            message:
              "No tasks found. The tasks.json file hasn't been created yet. Run shrimp in this folder to generate tasks.",
          };
          res.writeHead(200, {
        ...
        const task880f = tasksData.tasks?.find(
          (t) => t.id === "880f4dd8-a603-4bb9-8d4d-5033887d0e0f"
        );
        if (task880f) {
      ...
      try {
        ...
        if (!memoryExists) {
          console.log(
            `[History] Memory directory does not exist at: ${memoryDir}`
          );
          res.writeHead(200, { "Content-Type": "application/json" });
          res.end(
            JSON.stringify({
        ...
        const memoryFiles = files.filter(
          (f) => f.startsWith("tasks_memory_") && f.endsWith(".json")
        );

        console.log(
          `[History] Found ${memoryFiles.length} memory files in ${memoryDir}`
        );
          ...
          memoryFiles.map(async (filename) => {
            try {
              ...
              if (timestampMatch) {
                ...
                timestamp = rawTimestamp.replace(
                  /T(\d{2})-(\d{2})-(\d{2})$/,
                  "T$1:$2:$3"
                );
              }
      ...
      if (
        !filename.startsWith("tasks_memory_") ||
      ...
      if (req.method === "GET") {
        ...
        try {
          ...
          if (success) {
            ...
            res.end(
              JSON.stringify(
                defaultTemplate || { message: "Template reset to default" }
              )
            );
          } else {
            res.writeHead(500, { "Content-Type": "text/plain" });
      ...
      req.on("end", async () => {
        try {
          ...
          const success = await saveCustomTemplate(
            newName,
      ...
      url.pathname.startsWith("/api/agents/project/") &&
      req.method === "GET" &&
      url.pathname.split("/").length === 5
    ) {
      ...
      url.pathname.startsWith("/api/agents/global/") &&
      req.method === "GET"
    ) {
      ...
      url.pathname.startsWith("/api/agents/combined/") &&
      req.method === "GET"
    ) {
      ...
      try {
        ...
        if (claudeFolderPath) {
          ...
          try {
            ...
            globalAgents = await Promise.all(
              agentFiles.map(async (filename) => {
                try {
                  const filePath = path.join(agentsDir, filename);
                  const content = await fs.readFile(filePath, "utf8");
        ...
        if (projectRoot) {
          ...
          try {
            ...
            projectAgents = await Promise.all(
              agentFiles.map(async (filename) => {
                try {
                  const filePath = path.join(agentsDir, filename);
                  const content = await fs.readFile(filePath, "utf8");
      ...
      url.pathname === "/api/ai-assign-agents" &&
      req.method === "POST"
    ) {
      ...
      req.on("end", async () => {
        try {
          ...
          let openAIKey =
            ...
            process.env.OPEN_AI_KEY_SHRIMP_TASK_VIEWER;
          ...
          if (!openAIKey) {
            ...
            res.end(
              JSON.stringify({
                ...
                message:
                  "To use AI agent assignment, please configure your OpenAI API key.",
                instructions: [
          ...
          const tasksData = await fs.readFile(
            project.path || project.filePath,
            "utf8"
          );
          const tasksJson = JSON.parse(tasksData);
          const allTasks = Array.isArray(tasksJson)
            ? tasksJson
            : tasksJson.tasks || [];
          ...
          const prompt =
            "You are an AI assistant helping to assign specialized agents to tasks.\n\nAvailable Agents:\n" +
            agentsList +
            "\n\nTasks to Assign:\n" +
            tasksList +
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are a helpful assistant that assigns agents to tasks based on their descriptions and capabilities. Always respond with valid JSON only.",
              },
              {
                role: "user",
                content: prompt,
          ...
          try {
            ...
            const dataToSave = Array.isArray(tasksJson)
              ? allTasks
              : { ...tasksJson, tasks: allTasks };
            await fs.writeFile(
              project.path || project.filePath,
      ...
      req.on("end", async () => {
        try {
          ...
          let systemPrompt =
            "You are an AI assistant helping with task management in the Shrimp Task Manager. You have access to information about available agents and can help users understand tasks, suggest agent assignments, and provide task-related insights.\n\n";
          ...
          if (context.currentTask) {
            ...
            systemPrompt +=
              "- Description: " +
              (context.currentTask.description || "No description") +
              "\n";
            systemPrompt +=
              "- Assigned Agent: " +
              (context.currentTask.assignedAgent || "Unassigned") +
              "\n";
            systemPrompt +=
              "- Dependencies: " +
              (context.currentTask.dependencies?.join(", ") || "None") +
              "\n";
          }
          ...
          if (context.completedTasks && context.completedTasks.length > 0) {
            systemPrompt += "\nCompleted Tasks:\n";
            systemPrompt += context.completedTasks
              .map(
                (t) =>
          ...
          if (context.inProgressTasks && context.inProgressTasks.length > 0) {
            systemPrompt += "\nIn Progress Tasks:\n";
            systemPrompt += context.inProgressTasks
              .map(
                (t) =>
                  "- " + t.name + (t.description ? ": " + t.description : "")
              )
          ...
          if (context.pendingTasks && context.pendingTasks.length > 0) {
            systemPrompt += "\nPending Tasks:\n";
            systemPrompt += context.pendingTasks
              .map(
                (t) =>
                  "- " + t.name + (t.description ? ": " + t.description : "")
              )
          ...
          if (context.availableAgents && context.availableAgents.length > 0) {
            systemPrompt += "\nAvailable Agents:\n";
            systemPrompt += context.availableAgents
              .map(
                (a) =>
                  "- " +
                  a.name +
          ...
          if (
            context.agentAssignments &&
            Object.keys(context.agentAssignments).length > 0
          ) {
            systemPrompt += "\nAgent Assignment Statistics:\n";
            systemPrompt += Object.entries(context.agentAssignments)
              .map(
                ([agent, stats]) =>
                  "- " +
                  agent +
          ...
          if (context.unassignedTasks && context.unassignedTasks.total > 0) {
            systemPrompt +=
              "\nUnassigned Tasks: " +
              context.unassignedTasks.total +
              " total (" +
              context.unassignedTasks.completed +
              " completed, " +
              context.unassignedTasks.inProgress +
              " in progress, " +
              context.unassignedTasks.pending +
              " pending)\n";
          }
          ...
          systemPrompt +=
            "IMPORTANT: If the user asks to modify/edit a task and there is a currentTask in the context, respond with the modification in this EXACT format:\n";
          systemPrompt +=
            "TASK_MODIFICATION: {JSON object with the fields to update}\n\n";
          ...
          systemPrompt +=
            "- assignedAgent: Which agent is assigned to the task\n";
          ...
          systemPrompt +=
            'TASK_MODIFICATION: {"notes": "Updated notes with hello world"}\n';
          systemPrompt +=
            'TASK_MODIFICATION: {"description": "New description", "status": "in_progress"}\n';
          systemPrompt +=
            'TASK_MODIFICATION: {"assignedAgent": "gpt-engineer"}\n\n';
          systemPrompt +=
            "Be helpful, concise, and specific in your responses.\n\n";
          systemPrompt +=
            "FORMATTING: Use markdown formatting and emojis to make your responses more readable:\n";
          ...
          if (
            context.currentTask &&
            aiResponse.includes("TASK_MODIFICATION:")
          ) {
            try {
              // Extract the JSON from the response
              const modificationMatch = aiResponse.match(
                /TASK_MODIFICATION:\s*(\{[^}]+\})/
              );
  ...
  server.listen(PORT, "127.0.0.1", () => {
    ...
    if (projects.length === 0) {
      console.log(
        "  - No projects configured. Add projects via the web interface."
      );
    } else {
